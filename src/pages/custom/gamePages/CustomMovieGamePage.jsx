    ...
    const loadQuestions = async () => {
      ...
      try {
        ...
        const formatted = questions
          ...
          .map((question, idx) => ({
            id: question.id || `movie-${idx}`,
            imgUrl: question.imgUrl || question.questionText || '',
            answer: question.answer?.trim() || '',
          }))
          .filter((item) => item.imgUrl.length > 0);

        if (!formatted.length) throw new Error('커스텀 명대사 문제가 없습니다.');
        if (mounted) setCards(formatted);
      } catch (error) {
        console.error('커스텀 명대사 문제 불러오기 실패:', error);
        if (mounted) setCards(buildFallbackCards());
      }
    };
...
export default CustomMovieGamePage;
