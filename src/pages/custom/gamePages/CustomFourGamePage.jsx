    ...
    const loadQuestions = async () => {
      ...
      try {
        ...
        const formatted = questions
          ...
          .map((question, idx) => ({
            id: question.id || `four-${idx}`,
            name: question.questionText?.trim() || question.answer?.trim() || '',
          }))
          .filter((item) => item.name.length > 0);

        if (!formatted.length) throw new Error('커스텀 네글자 문제가 없습니다.');
        if (mounted) setCards(formatted);
      } catch (error) {
        console.error('커스텀 네글자 문제 불러오기 실패:', error);
        if (mounted) setCards(buildFallbackCards());
      }
    };
  ...
  return (
    <Container tabIndex="0" ref={focusRef} $transparent={isPreview}>
      ...
      <Content>
        ...
        <CardContainer>
          <Card>{cards[currentCardIndex].name} _ _</Card>
        </CardContainer>
