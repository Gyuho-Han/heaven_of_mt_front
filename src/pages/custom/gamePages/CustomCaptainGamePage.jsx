  ...
  useEffect(() => {
    ...
    const buildFallbackCards = () =>
      [...captainData]
        ...
        .slice(0, 10)
        .map((item, idx) => ({
          id: item.id || `fallback-captain-${idx}`,
          name: item.name,
          answer: item.answer,
        }));
    ...
    const loadQuestions = async () => {
      ...
      try {
        ...
        const formatted = questions
          ...
          .map((question, idx) => ({
            id: question.id || `captain-${idx}`,
            name: question.questionText?.trim() || '',
            answer: question.answer?.trim() || '',
          }))
          .filter((item) => item.name.length > 0 || item.answer.length > 0);

        if (!formatted.length) throw new Error('커스텀 대표게임 문제가 없습니다.');
        if (mounted) setCards(formatted);
      } catch (error) {
        console.error('커스텀 대표게임 문제 불러오기 실패:', error);
        if (mounted) setCards(buildFallbackCards());
      }
    };
...
export default CustomCaptainGamePage;
