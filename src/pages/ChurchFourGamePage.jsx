...
import { churchFourData } from '../gameData';
import ReadyPage from './ReadyPage';

const ChurchFourGamePage = () => {
  ...
  useEffect(() => {
    ...
    const shuffled = [...churchFourData].sort(() => 0.5 - Math.random());
  ...
  const handleKeyDown = useCallback((e) => {
    ...
    else if (e.key === 'ArrowLeft') { e.preventDefault(); if (currentCardIndex > 0) setCurrentCardIndex((p) => p - 1); }
    else if (e.key === 'ArrowRight') {
      e.preventDefault();
      if (currentCardIndex < cards.length - 1) setCurrentCardIndex((p) => p + 1);
      else navigate('/gameover', { state: { gameName: 'churchFour' } });
    }
  ...
  return (
    <Container tabIndex="0" ref={focusRef}>
      ...
      <Content>
        ...
        <NavButton onClick={() => currentCardIndex < cards.length - 1 ? setCurrentCardIndex((p) => p + 1) : navigate('/gameover', { state: { gameName: 'churchFour' } })}>
...
export default ChurchFourGamePage;
